...
public class Commodity {
    ...
    void buyCommodity(int stock_quantity, BigDecimal stock_price_bought){

        try{
            Class.forName("com.mysql.jdbc.Driver");
            Connection con= DriverManager.getConnection("jdbc:mysql://localhost:3306/StockMarketHandler","root","danimithil123");
            Statement stmt=con.createStatement();
            String query = "insert into stocksbought (Stock, Quantity, Price) values ('"+stock_name+"','"+stock_quantity+"','"+stock_price_bought+"');";
    ...
    void sellCommodity(BigDecimal stock_price_sold){
        ...
        try{
            ...
            query = "select * from stocksbought where Stock = '"+stock_name+"';";
            ResultSet rs = stmt.executeQuery(query);
            rs.next();
            int transID = rs.getInt(1);
            stock_quantity = rs.getInt(3);
            stock_price_bought = rs.getBigDecimal(4);
            stock_profit = (stock_price_sold.subtract(stock_price_bought).multiply(new BigDecimal(stock_quantity)));
            ...
            query = "insert into stockssold (TransID, Stock, Quantity, Price, Profit) " +
                    "values ('"+transID+"','"+stock_name+"','"+stock_quantity+"','"+stock_price_sold+"','"+stock_profit+"');";
            if ((stmt.executeUpdate(query)== 0)){
